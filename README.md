...
Приложение не подписано сертификатом Apple Developer, поэтому при первом запуске macOS может показать ошибку **«damaged and can't be opened»**.

Решение — выполните в Терминале:

```bash
xattr -cr /Applications/Meridian\ Mail.app
```
