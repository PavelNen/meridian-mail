...
![Meridian Mail](docs/screenshot.png)
...
Приложение подписано ad-hoc подписью (без сертификата Apple Developer). При первом запуске macOS покажет предупреждение «неизвестный разработчик» — нажмите правую кнопку на `.app` → **«Открыть»** → **«Открыть»**.
